...
async function loadAndProcessJSON() {
  try {
    ...
    const filteredData = data.filter((person) => person.name.startsWith("A")); // .startsWith solo debuelve un boleano dependiendo si el string que se le paso comienza con el ´parametro dado o no
